...
void *test(void *data) {
	...
	long rslt = rand_range_re(&d->seed, 100);
//	unext = (rand_range_re(&d->seed, 100) - 1 < d->update);
	unext = (rslt - 1 < d->update);
	printf("d->update : %d\n", d->update);
	printf("rand_range_re(&d->seed, 100) -1 < d->update) : %ld - 1 < %d\n", rslt, d->update);
	printf("unext : %d\n", unext);
#ifdef ICC
	//while (stop == 0) {
#else
...
/*	if (pthread_key_create(&rng_seed_key, NULL) != 0) {
	...
	node_t *node1 = set->head;
	int ge1 = 0;
	while(NULL != node1){
		printf("%d : %lu  ", ge1 , node1->key);
		ge1++;
		node1 = node1 -> next;
	}
	printf("\n");
