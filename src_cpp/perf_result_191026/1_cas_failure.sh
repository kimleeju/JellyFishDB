	...
	for wk in $workloads; do
		...
		for th in $threads; do
			...
			for sk in $skiplists; do
				...
				if [ "$th" -eq 1 ]; then
					head -n 22 "$srcfile" | grep "CAS" |  grep "$sk" |awk '{print $6}' | tr "\n" " " >> $datafile
				elif [ "$th" -eq 2 ]; then
					head -n 44 "$srcfile" | tail -n 22 | grep "CAS" |  grep "$sk" |awk '{print $6}' | tr "\n" " " >> $datafile 
				elif [ "$th" -eq 4 ]; then
					head -n 66 "$srcfile" | tail -n 22 | grep "CAS" |  grep "$sk" |awk '{print $6}' | tr "\n" " " >> $datafile
				elif [ "$th" -eq 8 ]; then
					head -n 88 "$srcfile" | tail -n 22 | grep "CAS" |  grep "$sk" |awk '{print $6}' | tr "\n" " " >> $datafile
				elif [ "$th" -eq 16 ]; then
					head -n 110 "$srcfile" | tail -n 22 | grep "CAS" |  grep "$sk" |awk '{print $6}' | tr "\n" " " >> $datafile						
				fi	
