...
Node* JellyFishSkipList::FindGreaterorEqual(const string& key){
    ...
    while(true){
		...
		if(cmp==0){
			assert(Comparator(key, next->Get_key())== 0);
			return next;
		}
        else if(cmp > 0 && level ==0){
            return next;
...
{
	cout << "JellyFishSkipList Number of comparison = " << n_cmp_read << endl;
	cout << "JellyFishSkipList Number of pointer refs = "<< n_ptr_ref << endl;
	cout << "JellyFishSkipList Number of CAS update = " << n_cas << endl;
	cout << "JellyFishSkipList Number of CAS fail = " << n_cas_fail << endl; 
}
