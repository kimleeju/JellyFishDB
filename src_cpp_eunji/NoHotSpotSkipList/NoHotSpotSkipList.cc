#include "NoHotSpotSkipList.h"

int NoHotSpotSkipList::Put(string key, string value, Iterator iterator){
...
string NoHotSpotSkipList::Get(string key , Iterator iterator){
//    t_global_committed.mlock.lock();
    t_global_committed.get_and_inc();
...
void NoHotSpotSkipList:::RangeQuery(string start_key, int count, Iterator iterator ){
    t_global_committed.mlock.lock();
    t_global_committed.get_and_inc();
    cout<<"-----------------------------"<<endl;
...
NoHotSpotSkipList::Splice* NoHotSpotSkipList::AllocateSplice(){
...
Node* NoHotSpotSkipList::FindLast(){
    Node* x = head_;
...
Node* NoHotSpotSkipList::FindLessThan(string key, Node** prev){
    int level = kMaxHeight_ -1 ;
    Node* x = head_;
...
Node* NoHotSpotSkipList::FindGreaterorEqual(string key){
    Node* x = head_;
    int level = kMaxHeight_ -1;
...
int NoHotSpotSkipList::RecomputeSpliceLevels(string key, int level, Splice* splice){
    Node* before = head_;
...
void NoHotSpotSkipList::FindSpliceForLevel(string key, int level, Node** sp_prev, Node** sp_next, Node* before){
...
bool NoHotSpotSkipList::KeyIsAfterNode(string key, Node* n){
...
Node* NoHotSpotSkipList::AllocateNode(string key, string value, int height){
...
int NoHotSpotSkipList::RandomHeight(){
...
NoHotSpotSkipList::NoHotSpotSkipList()
...
bool NoHotSpotSkipList::Insert(string key, string value, Iterator iterator)
{
	// put <k, v>
