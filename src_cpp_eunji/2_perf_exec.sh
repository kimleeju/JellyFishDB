...
#
		...
		while [[ $th -le $THREADS ]]; do
			for sk in $SKIPLISTS; do
				iter=5
				while [[ $iter -le 5 ]]; do
					rfname="cache_misses/"$sk"_"$op"_"$cf".perf"
					...
					#./Run $sk $th $load_trc $run_trc 
					rfname=$rfname.stat
					perf stat -o $rfname ./Run $sk $th $load_trc $run_trc 
					#perf record -e cache-misses,cache-references ./Run $sk $th $load_trc $run_trc 
					#perf report > $rfname
